...
class Spreadsheet_Excel_Writer_Parser extends PEARS
{
    /**
    ...
    * @return mixed the converted token on success. PEARS_Error if the token
    *               is not recognized
    */
    ...
    * @return mixed the converted token on success. PEARS_Error if the string
    ...
    */
    function _convertRange2d($range)
    {
        ...
        if (preg_match("/^([A-Ia-i]?[A-Za-z])(\d+)\:([A-Ia-i]?[A-Za-z])(\d+)$/", $range)) {
            ...
            // TODO: use real error codes
            return $this->raiseError("Unknown range separator", 0, PEARS_ERROR_DIE);
        }

        // Convert the cell references
        $cell_array1 = $this->_cellToPackedRowcol($cell1);
        if (PEARS::isError($cell_array1)) {
            return $cell_array1;
        }
        list($row1, $col1) = $cell_array1;
        $cell_array2 = $this->_cellToPackedRowcol($cell2);
        if (PEARS::isError($cell_array2)) {
            return $cell_array2;
        }
        ...
        if ($class == 0) {
            ...
            // TODO: use real error codes
            return $this->raiseError("Unknown class $class", 0, PEARS_ERROR_DIE);
        }
    ...
    * @return mixed The packed ptgArea3d token on success, PEARS_Error on failure.
    */
    ...
    * @access private
    * @param string $cell An Excel cell reference
    * @return mixed The packed ptgRef3d token on success, PEARS_Error on failure.
    */
    ...
    *                        sign (=).
    * @return mixed true on success, PEARS_Error on failure
    */
    function parse($formula)
    {
        ...
        if (PEARS::isError($this->_parse_tree)) {
    ...
    *
    * @access private
    * @return mixed The parsed ptg'd tree on success, PEARS_Error on failure
    */
    function _condition()
    {
        $result = $this->_expression();
        if (PEARS::isError($result)) {
            return $result;
        }
        if ($this->_current_token == SPREADSHEET_EXCEL_WRITER_LT) {
            $this->_advance();
            $result2 = $this->_expression();
            if (PEARS::isError($result2)) {
                return $result2;
            }
    ...
    */
    function toReversePolish($tree = array())
    {
        ...
        if (is_array($tree['left'])) {
            ...
            if (PEARS::isError($converted_tree)) {
