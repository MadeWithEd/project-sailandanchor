...
GLOBAL $search,$CONFIG,$Config,$_REQUEST,$orderby;
	...
	$SD["header"]["fields"]["content"]=array("","{page_name}","{page_file_name}","#- if('{sub_id}'==247) { echo 'Our Pub'; } else { echo 'Whats On'; } -#","{last_updated}","<a href=\"".FCPATHNICE."?mod=".$_REQUEST["mod"]."&task=create&id={page_id}\"><img src=\"".TEMPLATEPATHNICE."images/ico_edit.gif\" border=\"0\" alt=\"\" /></a>");
	$SD["query"]["select"]="select *";
	$SD["query"]["from"]="pages";
	$SD["query"]["where"]="1 AND (sub_id='247' OR sub_id='248') $WHERE";
...
+"var[class|dir<ltr?rtl|id|lang|onclick|ondblclick|onkeydown|onkeypress"
  ...
  +"|title]",
	...
	<h1><?=($_REQUEST["id"]>0)?'Editing '.$Details["page_name"]:'Add New Content';?></h1>
	...
	<input type="hidden" name="mandatory" value="page_name,page_html" />
		...
		<div id="one_panel" class="panel<?=(!isset($_REQUEST["step"]))?' current':'';?>">
			...
			<div<?=HighlightMandatory("sub_id");?>>
				...
				<option value="247"<?=ShowSelected('247',"sub_id",$Details);?>>Our Pub</option>
				<option value="248"<?=ShowSelected('248',"sub_id",$Details);?>>What's On</option>
				</select>
			</div>
