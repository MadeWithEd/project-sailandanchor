...
switch($task) {
	...
	case "save":
		...
		if(!$MISSING = CheckMandatoryFields()) {
			...
			if(is_uploaded_file($_FILES["page_background"]["tmp_name"])) {
				$filename=base_convert( md5(microtime()), 10, 36 );
				$ext='.'.strtolower(substr(strrchr($_FILES["page_background"]["name"],'.'), 1));
				if(copy($_FILES["page_background"]["tmp_name"],$Config["content"]["images"].$filename.$ext)) {
				$FD=@array_merge($FD,array("page_background" => $filename.$ext));
				}
				else {
				$error=true;
				}
			...
			if(isset($_POST["remove_current_background"])) {
			$FD=@array_merge($FD,array("page_background" => ""));
			}
